...
main = runTest do
    ...
    suite "GameGameState" do
        ...
        test "Black uses very last disk on first move (contrived)" do      
            let 
                ...
                history1 = swapLast makeHistory $ SequenceState (seqRec initialSequenceState) {game = taggedGameState1}
                moves1 = nextMoves_FromTaggedGameState taggedGameState1  
                move1 = unsafePartial $ fromJust $ head moves1 

                history2 = unsafePartial $ fromRight $ applyMoveOnHistory move1 history1 
                (EndedGameState {priorMove: _, status: endReason, core: _}) = unsafeEndedGameStateFrom $ (seqRec (NE.last history2)).game
    ...
    suite "Validate Move" do
        ...
        test "Undo (actual game)" do
            ...
            Assert.equal' "forfeits" (NE.filter isForfeitTurn $ map (\(SequenceState rec) -> rec.game) history26) $ fromFoldable [taggedGameState21, taggedGameState22]                                                       
